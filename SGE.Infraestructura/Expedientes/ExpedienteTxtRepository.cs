...
public sealed class ExpedienteTxtRepository : IExpedienteRepository
{
    ...
    {
        List<Expediente> miLista = ObtenerTodos().ToList();
        var expediente = miLista.FirstOrDefault(x => x.Id == id);
        if (expediente == null)
        {
            throw new RepositoryException("El expediente a modificar no se encuentra en el repositorio.");
        }
        miLista.Remove(expediente);
    ...
    {
        List<Expediente> miLista = ObtenerTodos().ToList();
        var expedienteViejo = miLista.FirstOrDefault(x => x.Id == e.Id);
        ...
        miLista.Remove(expedienteViejo);
